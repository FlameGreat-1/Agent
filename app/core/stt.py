...
class SpeechToTextService:
    ...
    def __init__(self, model_path: str = settings.WHISPER_MODEL_PATH,
        ...
        logger.info(f"Initialized STT service with model availability: {self.model_available}")
    ...
    def _validate_model(self) -> bool:
        ...
        if not os.path.exists(self.model_path):
            warning_msg = f"Warning: Whisper model not found at {self.model_path}"
    ...
    def transcribe(self, audio_data: BinaryIO, language: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not self.model_available:
            logger.warning(f"Transcription job {job_id} skipped: Whisper model not available")
            return {
                "job_id": job_id,
                "text": "Speech-to-text is unavailable (model not found)",
